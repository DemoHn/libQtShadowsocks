...
using namespace QSS;
AES::AES(const QByteArray &key, const QByteArray &iv, QObject *parent) : QObject(parent)
{
    aes_key = key;
    if(aes_key.size() == 256 / 8)
    {
        keyLength = KeyBits::B256;
    }else if(aes_key.size() == 192 / 8)
    {
        keyLength = KeyBits::B192;
    }else if(aes_key.size() == 128 / 8)
    {
        keyLength = KeyBits::B128;
    }else{
        keyLength = KeyBits::B128;
    }
...
QByteArray AES::decode(QByteArray &input)
{
    ...
    while(1)
    {
        truncate_length = decode_data.data()[len - 1];

        if(i == truncate_length){
            break;
        }
        //check if the data is right
        if(decode_data.data()[len - 1 - i] == truncate_length)
        {
            i += 1;
        }else{
            //padding data is wrong
            truncate_length = 0;
            break;
        }
    }

    decode_data.resize(len - truncate_length);
    return decode_data;
}
...
void AES::XOR(uint8_t *a, uint8_t *b, int count, uint8_t *result)
{
 ...
 */
uint8_t AES::gadd(uint8_t a, uint8_t b) {
        return a^b;
}
 ...
 * http://en.wikipedia.org/wiki/Finite_field_arithmetic
 */
uint8_t AES::gsub(uint8_t a, uint8_t b) {
        return a^b;
}
